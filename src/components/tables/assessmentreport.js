  ...
  const AdvancedSearch = async (data) => {
    ...
    try {
      ...
      for (let i = 0; i < res.length; i++) {
        ...
        rec.totalTaxFormated = formatNumber((Number(rec.add_assmt) + Number(rec.tax) + Number(rec.dev_levy)))
        rec.totalTaxDue = (Number(rec.add_assmt) + Number(rec.tax) + Number(rec.dev_levy))
